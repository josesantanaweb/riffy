'use client';
import { mapPaymentStatusToStatusType, mapPaymentStatusToLabel } from '@/utils';
...
const LastPayments = ({ payments }: LastPaymentsProps) => {
  return (
    <div className="flex-col bg-box-primary rounded-xl p-6 min-h-[300px] hidden md:flex">
      <h3 className="text-base font-medium text-title mb-6">Ultimos pagos</h3>
      {payments.length === 0 && (
      ...
      {payments.length > 0 && (
        <div className="flex flex-col">
          ...
          {payments.map(payment => (
            <div
              className="items-center py-2 grid grid-cols-4 md:grid-cols-5"
              key={payment.id}
            >
